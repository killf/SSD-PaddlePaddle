...
class COCOAnnotationTransform(object):
    ...
    def __init__(self):
        self.label_map = cfg.COCO_LABEL_MAP
    ...
    def __call__(self, target, width, height):
        ...
        for obj in target:
            if 'bbox' in obj:
                ...
                final_box = list(
                    np.array([bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]], dtype=np.float32) / scale)
...
class COCODetection:
    ...
    def __init__(self, image_path, info_file, batch_size=None, transform=None,
                 target_transform=None, dataset_name='MS COCO', has_gt=True):
...
def enforce_size(img, targets, masks, num_crowds, new_w, new_h):
    ...
    # Do all the resizing
    img = cv2.resize(img, (h_prime, w_prime))

    # Act like each object is a color channel
    masks = [cv2.resize(m, (h_prime, w_prime)) for m in masks]
    ...
    masks = [np.pad(img, pad_dims, 'constant', constant_values=0) for m in masks]
